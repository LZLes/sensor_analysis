...
**Desktop-app scope: Amperometry, Solid-State, and Cyclic Voltammetry only.** Assay is deliberately not offered in `macos_app/` (dropped from `main_window.py`'s `_MODES` and there is no `assay_view.py`) — it remains fully available in the Streamlit app (`modes/assay.py`, `app.py`), untouched. `AppState.data`'s `assay_*` fields still exist in `macos_app/ui/app_state.py`'s `SessionData` even though no macOS-app UI writes them — they must stay so `macos_app/persistence.py`'s Export/Import JSON bundle shape stays byte-compatible with the Streamlit app's, which does still have Assay.
...
- `macos_app/ui/widgets/` — shared panels reused across modes the same way Streamlit's `core/shared_tabs.py` is: `import_panel.py` (Browse/drag-and-drop/Load-sample + units only — auto-applies parsed files with no separate "Apply" gate — plus a read-only raw-CSV preview table next to the loaded-files list), `timeseries_panel.py` (the time-series plot/smoothing/channel-visibility checklist, **and** `_ChannelRow`/`_FileChannelEditor` for per-file channel-name/time-col/signal-col assignment — moved here from `import_panel.py` so channel mapping happens once the trace is already visible, not before), `autodetect_panel.py` (Amperometry + Solid-State), `plot_view.py` (a native pyqtgraph chart widget — lines/markers/error-bars/shaded calibration-window regions/vlines/hlines, a linked range-slider overview strip, and a unified crosshair-hover readout — used by every mode; **not** Plotly/QWebEngineView, see that module's docstring for why), `editable_table_view.py`/`pandas_table_model.py` (the `st.data_editor` replacement), `comparison_view.py` (cross-file overlay — new relative to the Streamlit app, not a port), `collapsible.py` (tiny `make_collapsible(box, content, expanded)` helper wrapping a checkable `QGroupBox`, used everywhere a secondary control group should start out of the way instead of permanently expanded).
- `macos_app/ui/dialogs/export_options_dialog.py` — `ExportOptionsDialog`, a shared Format(PNG/SVG/PDF/TIFF)/DPI/Style(Default/Origin/Minimal)/figure-size picker used by every PNG export action across all three modes, wired to the `dpi`/`fmt`/`figsize`/`style` kwargs `render_ts_png`/`render_cal_png`/`render_solid_cal_png` (and CV's own local PNG builders, see below) already accepted but the app didn't expose before. Also renders a live preview (always rasterized as PNG regardless of the chosen export format) that re-renders on every control change — callers pass a `render_fn(opts) -> bytes` closure that's the same one they use to build the real export bytes after the dialog closes, so the preview and the saved file are guaranteed to match.
- `macos_app/ui/modes/*_view.py` — one per mode. Amperometry and Solid-State are a 3-tab pipeline: ① Import (lean — file loading + units only) → ② Time Series & Calibration (a vertical split: the time-series plot + collapsible side groups for channel assignment/calibration-windows table+autodetect/Amperometry's effective-concentration dilution calculator, all next to the trace they affect; below that, fit settings + Compute button + the calibration plot/stats, reading its channel selection from the time-series visibility checklist above rather than a second duplicate list — this tab merges what used to be two separate tabs) → ③ Export (via the dialog above), plus a 4th Compare Files tab. Cyclic Voltammetry is self-contained (doesn't reuse the Amperometry/Solid-State panels — `modes/cyclic_voltammetry.py`'s own docstring calls its inline CSV parsing "pre-existing duplication, not part of scope" to fix) but follows a similar shape: ① Import (column-mapping details collapsed by default), ② Plot & Peaks (merged — running peak detection re-renders the same CV plot the markers land on, instead of a separate tab), ③ Scan Rate Analysis, ④ Export.
...
Charts used to be Plotly HTML embedded in a `QWebEngineView` (hence `macos_app/main.py` used to set `QApplication.AA_ShareOpenGLContexts` before constructing `QApplication` — Qt's prerequisite for that widget). That approach is gone: `QWebEngineView.setHtml()` silently drops any document over ~2MB, and a `full_html` Plotly export with `plotly.js` embedded is routinely 4MB+, so every plot came up blank; separately, an embedded browser view never looked native. `plot_view.py`'s `PlotView` is now a native pyqtgraph widget instead, and `QtWebEngineWidgets`/`AA_ShareOpenGLContexts` are gone from `macos_app/` entirely — don't reintroduce either without solving both problems some other way.
