...
- `macos_app/ui/widgets/` — shared panels reused across modes the same way Streamlit's `core/shared_tabs.py` is: `import_panel.py` (Browse/drag-and-drop/Load-sample + units only — auto-applies parsed files with no separate "Apply" gate), `timeseries_panel.py` (the time-series plot/smoothing/channel-visibility checklist, **and** `_ChannelRow`/`_FileChannelEditor` for per-file channel-name/time-col/signal-col assignment — moved here from `import_panel.py` so channel mapping happens once the trace is already visible, not before), `autodetect_panel.py` (Amperometry + Solid-State), `plot_view.py` (Plotly-in-`QWebEngineView`, used by every mode), `editable_table_view.py`/`pandas_table_model.py` (the `st.data_editor` replacement), `comparison_view.py` (cross-file overlay — new relative to the Streamlit app, not a port), `collapsible.py` (tiny `make_collapsible(box, content, expanded)` helper wrapping a checkable `QGroupBox`, used everywhere a secondary control group should start out of the way instead of permanently expanded).
- `macos_app/ui/dialogs/export_options_dialog.py` — `ExportOptionsDialog`, a shared Format(PNG/SVG/PDF/TIFF)/DPI/Style(Default/Origin/Minimal)/figure-size picker used by every PNG export action across all three modes, wired to the `dpi`/`fmt`/`figsize`/`style` kwargs `render_ts_png`/`render_cal_png`/`render_solid_cal_png` (and CV's own local PNG builders, see below) already accepted but the app didn't expose before.
- `macos_app/ui/modes/*_view.py` — one per mode. Amperometry and Solid-State are a 4-tab pipeline: ① Import (lean — file loading + units only) → ② Time Series & Windows (the time-series plot plus, in collapsible side groups, channel assignment, the calibration-windows table + autodetect, and — Amperometry only — the effective-concentration/dilution calculator, all next to the trace they affect) → ③ Calibration Results (fit settings + Compute button + the calibration plot/stats, reading its channel selection from ②'s visibility checklist rather than a second duplicate list) → ④ Export (via the dialog above), plus a 5th Compare Files tab. Cyclic Voltammetry is self-contained (doesn't reuse the Amperometry/Solid-State panels — `modes/cyclic_voltammetry.py`'s own docstring calls its inline CSV parsing "pre-existing duplication, not part of scope" to fix) but follows the same shape: ① Import (column-mapping details collapsed by default), ② Plot & Peaks (merged — running peak detection re-renders the same CV plot the markers land on, instead of a separate tab), ③ Scan Rate Analysis, ④ Export.
...
`macos_app/main.py` sets `QApplication.setAttribute(Qt.AA_ShareOpenGLContexts, True)` **before** constructing `QApplication` — Qt's documented prerequisite for `QWebEngineView` (every `PlotView` embeds one). Without it, `QtWebEngine` can construct but silently never paint; don't remove this or reorder it after `QApplication(sys.argv)`.
